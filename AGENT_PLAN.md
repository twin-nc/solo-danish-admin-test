...
### Researcher Agent
Researches Danish tax administration — with a focus on Danish VAT (moms) — and provides continuous input on requirements from legal, procedural, and practical perspectives. Runs at every phase and iterates with the Architect and Designer agents to ensure the platform reflects real Danish tax law and SKAT procedures.
...
### Reviewer Agent
Reviews all Phase 1 outputs (Architect, Designer, Researcher) as a critic and quality gate before any code is written. Does not redesign — interrogates accuracy, coherence, and decision justification across all three specs.

**Responsibilities:**
- Verify that the Architect's data model correctly incorporates the Researcher's legal findings (e.g. Rubrik A-E fields, CVR/SE numbers, afregningsperiode deadlines)
- Verify that the Designer's UI forms and fields are consistent with the Architect's schemas (no field shown in UI that does not exist in the API)
- Challenge the accuracy of domain knowledge in all specs: terminology, legal correctness, procedural correctness per Momsloven and SKAT procedures
- Identify decisions that are stated but not justified, and flag them for resolution
- Identify gaps where one agent's output assumes something another agent has not defined
- Produce a structured verdict: pass, flag, or reject for each major section across all three specs
- Do not propose wholesale redesigns — focus on correctness and coherence of what exists

**Input:** `AGENT_ARCHITECT_SPEC.md`, `AGENT_DESIGN_SPEC.md`, `AGENT_RESEARCHER_SPEC.md`
**Output:** `AGENT_REVIEWER_SPEC.md`
**Blocks:** Phase 2 does not start until Reviewer has signed off
...
Phase 1:
  ...
  Researcher Agent  ──→  AGENT_RESEARCHER_SPEC.md
                         (reviews Architect + Designer specs; provides legal/procedural feedback)
```

### Phase 1 Review — After Phase 1
Run Reviewer Agent against all Phase 1 outputs. Phase 2 does not start until this is complete.

```
Phase 1 Review:
  Reviewer Agent  ──→  AGENT_REVIEWER_SPEC.md
                        (reviews Architect + Designer + Researcher specs)
```

### Phase 2 — After Phase 1 Review
Run Frontend Coder and Backend Coder agents in parallel. Both consume Phase 1 outputs and Reviewer findings.
...
```
Researcher Agent ──→ AGENT_RESEARCHER_SPEC.md ──┐
Architect Agent  ──→ AGENT_ARCHITECT_SPEC.md  ──┼──→ Reviewer Agent ──→ AGENT_REVIEWER_SPEC.md ──┐
Designer Agent   ──→ AGENT_DESIGN_SPEC.md     ──┘                                                 │
                                                                                                   ├──→ Frontend Coder Agent ──→ Test Agent ──→ Docs Agent
                                                                                                   └──→ Backend Coder Agent  ──→ Test Agent ──→ Docs Agent
```

The Researcher Agent runs at Phase 1 and its output feeds into the Reviewer. The Reviewer acts as the gate between Phase 1 and Phase 2 — all Coder agents receive both the original specs and the Reviewer's findings.
...
├── AGENT_RESEARCHER_SPEC.md      # Generated by Researcher Agent
├── AGENT_REVIEWER_SPEC.md        # Generated by Reviewer Agent
└── AGENT_ROLES.md                # Agent role definitions (all agents)
```
